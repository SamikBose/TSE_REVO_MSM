...
TS_labels_path = f'{base_dir}/lag1_TSE/TS_labels_NOTICA'
...
cluster_list = [250,500,800,1200]
lag_time_list = [1]
#lag_time_list = [100]

model_base_path = {}
model_base_path['10'] = f'{base_dir}/MSM_models_withoutTICA'
model_base_path['17'] = f'{base_dir}/MSM_models_withoutTICA'
model_base_path['28'] = f'{base_dir}/MSM_models_withoutTICA'
model_base_path['29'] = f'{base_dir}/MSM_models_withoutTICA'
model_base_path['50'] = f'{base_dir}/MSM_models_withoutTICA'
model_base_path['19'] = f'{base_dir}/MSM_models_withoutTICA'
model_base_path['51'] = f'{base_dir}/MSM_models_withoutTICA_51/MSM_models_withoutTICA/'
...
for cutoff in cut_list:
    ...
    for lig in lig_list:
        ...
        for clust in cluster_list:
                model_path = f'{model_base_path[lig]}/model_{clust}/'

                for lag_time in lag_time_list:
                    
                    print(f'Running for lig{lig}, n_clust{clust}, lag{lag_time}...')
                    ...
                    if osp.exists(comm_path) and osp.exists(all_counts_path) and osp.exists(clusterlabels_path):
                        ...
                        if lig in comm.keys():
                            ...
                            print(f'Committor cutoff: {cut1},{cut2}.. Lig:{lig}, n_clust:{clust}, lag:{lag_time}, Num of states in the comm range:{count}\n')
                            ...
                            with open(f'{TS_labels_path}/TS_labels_lig{lig}_tau{lag_time}_clust{clust}_comm_cuts_{cut1}_{cut2}.pkl','wb') as f:
                                pkl.dump(state_label, f)
                            with open(f'{TS_labels_path}/TS_wts_lig{lig}_tau{lag_time}_clust{clust}_comm_cuts_{cut1}_{cut2}.pkl','wb') as f:
                                pkl.dump(state_wts, f)
                            '''
                            with open(f'{TS_labels_path}/all_state_wts_lig{lig}_tau{lag_time}_clust{clust}_comm_cuts.pkl','wb') as f:
                        ...
                        print(f'No comm file for lig{lig}_tau{lag_time}_clust{clust} ')
